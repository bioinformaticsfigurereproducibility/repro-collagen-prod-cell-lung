# ==============================================================================
# Download the mouse lung scRNA-seq data (GEO: GSE132771) this project uses
...
samples_name <- c(
  "Bleo1_GFPp", "Bleo1_GFPn", "Bleo2_GFPp", "Bleo2_GFPn", "UT1_GFPp", "UT1_GFPn",
  "UT2_GFPp", "UT2_GFPn"
)

raw_url <- "https://ftp.ncbi.nlm.nih.gov/geo/series/GSE132nnn/GSE132771/suppl/GSE132771_RAW.tar"
raw_tar <- here("reproduce_figure1", "data", "GSE132771_RAW.tar")
extract_dir <- here("reproduce_figure1", "data", "_raw_extracted")

dir.create(here("reproduce_figure1", "data"), showWarnings = FALSE)
...
for (s in samples_name) {
  sample_dir <- here("reproduce_figure1", "data", s)
  dir.create(sample_dir, showWarnings = FALSE)
  
  matches <- all_files[grepl(paste0("_", s, "_"), all_files, fixed = FALSE)]
  
  if (length(matches) == 0) {
    warning("No files matched for sample '", s, "' - check reproduce_figure1/data/_raw_extracted manually.")
    next
  }
